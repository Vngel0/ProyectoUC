...
# ocupacion_vi graficos ----

ocupacion_v1 %>%
  group_by(tipo_de_cama) %>%
  summarise(sum(casos_confirmados)) %>%
  ggplot(aes(tipo_de_cama))+
  geom_point()

# Evolucion de casos confirmados
ggplot(ocupacion_v1, aes(fecha, casos_confirmados , colour = tipo_de_cama))+
  geom_line() + theme(panel.grid.major = element_line(colour = "gray0",
    linetype = "blank"), panel.grid.minor = element_line(colour = "green4",
    ...
    strip.background = element_rect(colour = NA,
        linetype = "dashed"), strip.text = element_text(colour = "gray0",
        hjust = 1)) +labs(title = "Evolución de casos confirmados",
    x = "Fecha", y = "Casos Confirmados",
    colour = "Tipo de cama", caption = "Elaboracion propia")
...
vacunacion_pais_v1 %>%
  na.omit() %>%
  group_by(pais) %>%
  summarise(sum(vacunacion_diaria))

vacunacion_pais_v1 %>%
  ggplot(aes(pais, sum(vacunacion_total)), colour = pais)+
  geom_area()
